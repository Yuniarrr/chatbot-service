...
async def main():
    ...
    df.to_csv(
        f"{PARENT_DIR}/notebooks/evaluate/retriever/hybrid_search_cross_encoder/evaluation_result_hybrid_search_cross_encoder.csv",
        index=False,
    )
    df.to_excel(
        f"{PARENT_DIR}/notebooks/evaluate/retriever/hybrid_search_cross_encoder/evaluation_result_hybrid_search_cross_encoder.xlsx",
        index=False,
    )

    pd.DataFrame(evaluation_data).to_csv(
        f"{PARENT_DIR}/notebooks/evaluate/retriever/hybrid_search_cross_encoder/raw_evaluation_data_hybrid_search_cross_encoder.csv",
        index=False,
    )

    average_scores = df.mean(numeric_only=True)
    average_scores.to_csv(
        f"{PARENT_DIR}/notebooks/evaluate/retriever/hybrid_search_cross_encoder/average_scores_hybrid_search_cross_encoder.csv"
    )
    ...
    df.to_csv(
        f"{PARENT_DIR}/notebooks/evaluate/retriever/hybrid_search_cross_encoder/evaluation_result_hybrid_search_cross_encoder_with_time.csv",
        index=False,
    )
    average_scores = df.mean(numeric_only=True)
    average_scores.to_csv(
        f"{PARENT_DIR}/notebooks/evaluate/retriever/hybrid_search_cross_encoder/average_scores_hybrid_search_cross_encoder_with_time.csv"
    )
