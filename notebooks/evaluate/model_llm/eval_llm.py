...
async def main():
    ...
    evaluation_data = []
    processing_times = []
    agent = chain_service.create_agent("llama")
    ...
    df.to_csv(
        f"{PARENT_DIR}/notebooks/evaluate/model_llm/llama/evaluation_result_llama.csv",
        index=False,
    )
    df.to_excel(
        f"{PARENT_DIR}/notebooks/evaluate/model_llm/llama/evaluation_result_llama.xlsx",
        index=False,
    )

    pd.DataFrame(evaluation_data).to_csv(
        f"{PARENT_DIR}/notebooks/evaluate/model_llm/llama/raw_evaluation_data_llama.csv",
        index=False,
    )

    average_scores = df.mean(numeric_only=True)
    average_scores.to_csv(
        f"{PARENT_DIR}/notebooks/evaluate/model_llm/llama/average_scores_llama.csv"
    )
    ...
    df.to_csv(
        f"{PARENT_DIR}/notebooks/evaluate/model_llm/llama/evaluation_result_llama_with_time.csv",
        index=False,
    )
    average_scores = df.mean(numeric_only=True)
    average_scores.to_csv(
        f"{PARENT_DIR}/notebooks/evaluate/model_llm/llama/average_scores_llama_with_time.csv"
    )
