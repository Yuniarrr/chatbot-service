...
async def setup_environment():
    ...
    vector_store_service._init_total_docs()
...
async def main():
    ...
    df.to_csv(
        f"{PARENT_DIR}/notebooks/evaluate/top_k/top_k_9/evaluation_result_top_9.csv",
        index=False,
    )
    df.to_excel(
        f"{PARENT_DIR}/notebooks/evaluate/top_k/top_k_9/evaluation_result_top_9.xlsx",
        index=False,
    )

    pd.DataFrame(evaluation_data).to_csv(
        f"{PARENT_DIR}/notebooks/evaluate/top_k/top_k_9/raw_evaluation_data_top_9.csv",
        index=False,
    )

    average_scores = df.mean(numeric_only=True)
    average_scores.to_csv(
        f"{PARENT_DIR}/notebooks/evaluate/top_k/top_k_9/average_scores_top_9.csv"
    )
    ...
    df.to_csv(
        f"{PARENT_DIR}/notebooks/evaluate/top_k/top_k_9/evaluation_result_top_9_with_time.csv",
        index=False,
    )
